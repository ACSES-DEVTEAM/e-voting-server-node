...
const createCandidate = async (req, res) => {
  ...
  try {
    ...
    const candidateExist = await Candidate.findOne({ indexNumber });
    if (candidateExist) {
        // return error
...
const getCandidates = async (req, res) => {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (!user) {
      return res
        .status(404)
        .json({
          error: "No such candidate from user",
          indexNumber: candidates[i].indexNumber,
        });
    }
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const contact = candidates[i].user.contact;
    sortedCandidates[i] = {
      bio,
      photo,
      name,
